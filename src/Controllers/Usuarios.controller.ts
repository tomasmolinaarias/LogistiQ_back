...
const UsuariosController = {
  ...
  crearUsuario: async (req: CustomRequest, res: Response): Promise<Response | void> => {
    const { nombre, email, password, dni, idRol } = req.body;

    try {
      const usuarioExistente = await Usuarios.findOne({ where: { email: email.toLowerCase() } });
      if (usuarioExistente) {
      ...
      } as CreationAttributes<Usuarios>);
 ...
 eliminarUsuario: async (req: CustomRequest, res: Response): Promise<Response | void> => {
  const { id } = req.body;
...
actualizarUsuario: async (req: CustomRequest, res: Response): Promise<Response | void> => {
  ...
  try {
    ...
    if (req.body.password) {
      const saltRounds = 10;
      actualizaciones.password_hash = await bcrypt.hash(req.body.password, saltRounds);
    }
    ...
    await registrarEnBitacora(
      ...
      "Usuarios",
      usuario.dni,
      `Usuario ${usuario.nombre} actualizado de ${JSON.stringify(datosAnteriores)} a ${JSON.stringify(actualizaciones)}`
    );
  ...
  // Activar usuario
  activarUsuario: async (req: CustomRequest, res: Response): Promise<Response | void> => {
    const { id } = req.body;
  ...
  // Desactivar usuario
  desactivarUsuario: async (req: CustomRequest, res: Response): Promise<Response | void> => {
    const { id } = req.body;
